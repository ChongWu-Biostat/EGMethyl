...
data.tmp2 = data.tmp2[data.tmp2[,12] > 5e-8,]
...
data.tmp2 = data.tmp[!duplicated(data.tmp[,2]),]
data.tmp2 = data.tmp2[data.tmp2[,18] > 5e-8,]
...
standard = standard[standard[,12] >5e-8,]
...
CMC2 = readRDS("scz2_CMC2.rds")
NTR = readRDS("scz2_NTR.rds")
METSIM = readRDS("scz2_METSIM.rds")
...
gene.used = intersect(gene.used,standard.gene)
...
enhancer.methy.sig[!enhancer.methy.sig %in% unique(c(enhancer.coding.sig,standard.sig,TWAS.sig))]
